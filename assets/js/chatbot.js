...
(function () {
  ...
  const STEPS = [
    {
      key: null,
      message:
        "Olá! 👋 Sou a Júlia, secretária virtual da <strong>Synexus Contabilidade</strong>. Vou te ajudar a abrir ou regularizar seu MEI com segurança. Bora começar?",
      type: "choice",
      choices: [
        "Sim, vamos lá! 🚀",
        "Tenho dúvidas primeiro 💬",
        "Me preocupo com valores 💵",
      ],
    },
    {
      key: "name",
      message:
        "Antes de eu te passar para nosso consultor, qual é o seu <strong>nome completo</strong>?",
      type: "text",
    ...
    },
    {
      key: "situation",
      message: (data) =>
        `Prazer, <strong>${first(data.name)}</strong>! Qual é a sua situação atual?`,
      type: "choice",
      choices: [
        "Ainda não tenho CNPJ",
        "Já tenho MEI, mas preciso de ajuda",
        "Quero migrar do MEI para empresa",
      ],
    },
    {
      key: "activity",
    ...
    },
    {
      key: "phone",
      message: (data) =>
        `Perfeito! Agora preciso do seu <strong>WhatsApp</strong> para nosso consultor falar com você.`,
      type: "tel",
  ...
  function appendBubble(role, html, animate = true) {
    ...
    if (role === "bot") {
      ...
      avatar.src =
        "https://d2v5dzlf3dactd.cloudfront.net/secretaria_virtual.jpg";
      avatar.alt = "Júlia";
  ...
  function handleChoice(step, choice) {
    ...
    if (step.key === null) {
      ...
      if (choice.startsWith("Tenho dúvidas")) {
        ...
        inputAreaEl.innerHTML = "";
        botSpeak(
          "Sem problemas! Nosso consultor vai tirar todas as suas dúvida. 😊",
          () => {
            renderStep(1);
          },
        );
      ...
      if (choice.startsWith("Me preocupo com valores")) {
        ...
        inputAreaEl.innerHTML = "";
        botSpeak(
          "Não se preocupe pois na Synexus nossa preocupação é te ajudar por um valor que irá caber no seu bolso, sabemos da realidade de quem é ou está querendo ser MEI. 💚",
          () => {
            renderStep(1);
          },
        );
  ...
  function submitLeadSilently(collectedData) {
    if (typeof window.submitLeadData === "function") {
      ...
      if (
        collectedData.situation &&
        collectedData.situation.includes("preciso de ajuda")
      ) {
        serviceName = "Consultoria MEI (Chatbot)";
      } else if (
        collectedData.situation &&
        collectedData.situation.includes("migrar do MEI")
      ) {
